package com.smartcampus.core.database.auth
...
/**
 * Управляет подключением и транзакциями для базы данных аутентификации SmartCampus.
 *
 * Этот объект инкапсулирует инициализацию соединения с базой данных, используя [HikariDataSource]
 * и библиотеку Exposed (версии v1). Он предоставляет метод для выполнения запросов к базе данных
 * в рамках транзакций и метод для закрытия пула соединений.
 *
 * Перед использованием необходимо вызвать метод [init] для настройки соединения.
 */
object SmartCampusAuthDb {
    private val log = LoggerFactory.getLogger(SmartCampusAuthDb::class.java)

    private var dataSource: HikariDataSource? = null

    /**
     * Экземпляр базы данных Exposed, используемый для выполнения транзакций.
    ...
    /**
     * Инициализирует соединение с базой данных аутентификации.
     *
     ...
     * Имя пула соединений по умолчанию устанавливается как "SmartCampusAuthPool".
     *
     * @param dbConfig Конфигурация приложения Ktor, содержащая параметры подключения к базе данных
     *                 для сервиса аутентификации. См. документацию [DataSourceFactory.createAndConfigureDataSource]
    ...
    /**
     * Выполняет предоставленный блок кода [block] внутри новой транзакции базы данных.
    ...
    /**
     * Закрывает пул соединений [HikariDataSource].
     *

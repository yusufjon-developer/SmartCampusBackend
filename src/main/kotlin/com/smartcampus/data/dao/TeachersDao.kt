...
class TeachersDao(private val db: SmartCampusDb) {
    ...
    suspend fun getTeachers(params: PageRequestParams): PaginatedResult<TeacherListItemDto> = db.query {
        val rows = TeachersTable
            .selectAll()
            .applyPaginationAndSorting(params, SortableFields.TEACHERS, TeachersTable.name)
            .map { it.toTeacherListItemDto() }
    ...
    suspend fun getTeacherSensitiveById(id: Int): TeacherSensitiveDto? = db.query {
        val row = TeachersInfoTable.selectAll().where { TeachersInfoTable.teacherId eq id }.singleOrNull() ?: return@query null
        row.toTeacherSensitiveDto()
    }
    ...
    suspend fun updateTeacher(id: Int, request: TeacherUpdateRequest, performingUserId: Int): TeacherDetailsDto? = db.query {
        ...
        TeachersTable.update({ TeachersTable.id eq id }) {
            ...
            if (request.birthday != null) it[TeachersTable.birthday] = LocalDate.parse(request.birthday) else if (request.birthday == null) { /* keep */ }
    ...
    private fun ResultRow.toTeacherSensitiveDto(): TeacherSensitiveDto =
        TeacherSensitiveDto(

    ...
    authentication {
        jwt("auth-jwt") {
            ...
            validate { credential ->
                if (credential.payload.getClaim("userId")?.asInt() != null &&
                    credential.payload.getClaim("username")?.asString()?.isNotEmpty() == true
                ) {
                    JWTPrincipal(credential.payload)
                } else {
                    this@configureSecurity.log.warn("auth-jwt validation failed: userId or username missing/invalid.")
                    null
                }
            }
            challenge { _, _ ->
        ...
        jwt("auth-jwt-admin") {
            ...
            challenge { _, _ ->
                ...
                call.respond(HttpStatusCode.Unauthorized, mapOf("error" to "Access denied or token invalid/expired. Administrator rights required."))
            }
        }
    }
